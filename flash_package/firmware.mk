...
$(strip $1)/br_bct_BR.bct: $(INSTALLED_KERNEL_TARGET) $(INSTALLED_NVDISP_INIT_TARGET) $(INSTALLED_TIANOCORE_TARGET) $(INSTALLED_EDK2_DTBO_TARGET) $(TOYBOX_HOST) $(FDTPUT_HOST) $(SMD_GEN_HOST)
	...
	cd $(strip $1); PYTHONDONTWRITEBYTECODE=1 $(TEGRAFLASH_PATH)/tegraflash.py \
		--chip 0x19 \
		--bl $(strip $(1))/nvtboot_recovery_cpu_t194.bin \
		--applet $(strip $(1))/mb1_t194_prod.bin \
		--cmd "sign" \
		--cfg $(strip $(1))/$(strip $(2)) \
		--odmdata $(strip $(5)) \
		--overlay_dtb $(strip $(1))/AndroidConfiguration.dtbo,$(subst $(SPACE),,$(foreach dtbo,$(strip $(6)),$(KERNEL_OUT)/arch/arm64/boot/dts/nvidia/$(dtbo),)) \
		--bldtb $(strip $(1))/$(strip $(4)) \
